...
bool do_response(HttpData& httpdata, HTTP_CODE ret)
{
    ...
    case HTTP_CODE::INTERNAL_ERROR:
        {
            httpdata.m_response = response_status_line(500, error_500_title.c_str());
    ...
    case HTTP_CODE::BAD_REQUEST:
        {
            httpdata.m_response = response_status_line(400, error_400_title.c_str());
            httpdata.m_response += response_header(httpdata);
            httpdata.m_response += format_string("%s\r\n", error_400_form.c_str());
            break;
        }
    case HTTP_CODE::NO_REQUEST:
        {
            httpdata.m_response = response_status_line(404, error_404_title.c_str());
            httpdata.m_response += response_header(httpdata);
            httpdata.m_response += format_string("%s\r\n", error_404_form.c_str());
            break;  
        }
    case HTTP_CODE::FORBIDDEN_REQUEST:
        {
            httpdata.m_response = response_status_line(403, error_403_title.c_str());
            httpdata.m_response += response_header(httpdata);
            httpdata.m_response += format_string("%s\r\n", error_403_form.c_str());
            break;
        }
    case HTTP_CODE::FILE_REQUEST:
        {
            httpdata.m_response = response_status_line(200, ok_200_title.c_str());
            httpdata.m_response += response_header(httpdata);        
            break;
        }
    default:
...
void HttpSever::onMessage(const TcpConnectionPtr& conn, Buffer* buff)
{
    ...
    {
        wtimer = conn->getLoop()->addTimer(std::bind(&TcpServer::removeConn, tcpServer_,conn),
